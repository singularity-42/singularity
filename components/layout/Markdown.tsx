        ...
        components={{
          td: ({ node, ...props }) => {
            ...
            let row = children.map((child: any) => {
              ...
              if (!isElement) {
                ...
                name={child?.children[0]?.value || ''}>{child?.children[0]?.value || ''}</Link>;
              }
            });
          ...
          h1: ({ node, ...props }) => {
            ...
            for (let i = 0; i < children.length; i++) {
              ...
              if (child?.type === 'element') newChildren.push(<Link key={i} name={child?.children[0]?.value || ''}>{child?.children[0]?.value || ''}</Link>);
            ...
            for (let i = 0; i < children.length; i++) {
              ...
              if (child?.type === 'element' && child?.tagName == "a") {
                if (child?.properties?.href?.startsWith('http')) // if link is http or https than do this
                  newChildren.push(<Link key={i} href={child?.properties?.href}>{child?.children[0]?.value || ''}</Link>); 
                else // if #{hash}  its hash than do this 
                  newChildren.push(<Link key={i} name={child?.children[0]?.value.replace('#', '') || ''}>{child?.children[0]?.value.replace('#', '') || ''}</Link>);
