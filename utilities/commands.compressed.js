...
const onCreate = async (t, a, n) => { let r = a.chat.id, i = (a.text || "").split(" ").slice(2).join(" "), o = (a.text || "").split(" ").slice(-1)[0], m = i.replace(o, "").trim(), d = y("confirmed", `${a.from?.id}`); if (d < 1) return s(t, `${r}`, 1); let l = x(n, m); if (l) return c(t, `${r}`, m); let p = { mail: "", location: "", tags: [], connections: [], confirmed: [a.from?.id || ""] }, u = "\n42", g = M(p, u), h = m, k = { category: null, path: null, date: null, markdown: u, metadata: p, name: h }; w(k), await onUpdate(t, m, g, n), f(t, `${r}`, m), await t.sendMessage(`${r}`, g), e(t, `${r}`, m) };
