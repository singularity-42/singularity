...
const getDatePath = (date) => {
    let date_path = date.split('.').reverse().join('/');
    return date_path;
}

const getPath = (name, date = null, entityType = null) => {
    const files = getFiles(name, date, entityType);
...
const getFiles = (name = '*') => {
    let file_path = path.join(__dirname, '..', 'docs', '**', `${name}.md`);
    // let files = glob.sync(file_path); ignore lower case and upper case
